...
if "%1"=="" (
    ...
    echo   run-chainlit  - Run Chainlit UI locally without Docker
    echo   run-whatsapp  - Run WhatsApp FastAPI webhook locally without Docker
...
if /i "%1"=="run-chainlit" (
    echo Starting Chainlit UI locally on http://localhost:8000...
    uv run chainlit run src/ai_companion/interfaces/chainlit/app.py --port 8000
    goto :eof
)

if /i "%1"=="run-whatsapp" (
    echo Starting WhatsApp FastAPI endpoint locally on http://localhost:8080...
