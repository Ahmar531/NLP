    ...
    async def analyze_image(self, image_data: Union[str, bytes], prompt: str = "") -> str:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Groq vision API failed ({api_err}). Returning default image description.")
                return "An attached image/photo uploaded by the user."

            return "An attached image/photo uploaded by the user."

        except Exception as e:
            self.logger.warning(f"Failed to analyze image ({e}). Returning fallback.")
            return "An attached image/photo uploaded by the user."
