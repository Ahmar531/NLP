...
# Ensure Chainlit temporary file storage directory exists
...
async def on_message(message: cl.Message):
    ...
    user_id = cl.user_session.get("user_id") or cl.user_session.get("id") or "chainlit_user"
    ...
    async with cl.Step(type="run"):
        async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
            ...
            output_state = await graph.aget_state(config={"configurable": {"thread_id": thread_id, "user_id": user_id}})
...
async def on_audio_end(elements):
    ...
    input_audio_el = cl.Audio(mime="audio/mpeg", content=audio_data)
    ...
    async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
        ...
        output_state = await graph.ainvoke(
            {"messages": [HumanMessage(content=transcription)], "user_id": user_id},
            {"configurable": {"thread_id": thread_id, "user_id": user_id}},
        )
    ...
    if audio_buffer:
        ...
        await cl.Message(content=output_state["messages"][-1].content).send()
